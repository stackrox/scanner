...
func (s *serviceImpl) analyzeLayers(registryURL string, image *types.Image, layers []string, headers map[string]string) error {
	var prevLayer string
	for _, layer := range layers {
		fullURL := fmt.Sprintf("%s/v2/%s/blobs/%s", registryURL, image.Remote, layer)
		err := clair.ProcessLayer(s.db, "Docker", layer, prevLayer, fullURL, headers)
		if err != nil {
...
func (s *serviceImpl) process(image *types.Image, reg types.Registry) (string, string, error) {
...
func (s *serviceImpl) parseV1Layers(manifest *schema1.SignedManifest) []string {
	var layers []string
...
func (s *serviceImpl) parseV2Layers(manifest *schema2.DeserializedManifest) []string {
	var layers []string
...
func (s *serviceImpl) handleManifest(reg types.Registry, manifestType string, remote, ref string) ([]string, error) {
	switch manifestType {
...
func (s *serviceImpl) fetchLayers(reg types.Registry, image *types.Image) (string, []string, error) {
	ref := image.Tag
	if image.SHA != "" {
		ref = image.SHA
	}
