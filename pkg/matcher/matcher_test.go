...
import (
	mock2 "github.com/stackrox/scanner/pkg/fsutil/fileinfo/mock"
	"os"
	"regexp"
	"testing"
...
func TestPrefixAllowlistMatcher(t *testing.T) {
	...
	match, extract := m.Match("Test", mock2.NewFileInfo(), nil)
	...
	// Should not match.
	m = NewPrefixAllowlistMatcher("Test", "test")
	match, extract = m.Match("Fail", mock2.NewFileInfo(), nil)
	assert.False(t, match)
	assert.False(t, extract)
}
...
func TestWhiteoutMatcher(t *testing.T) {
	...
	match, extract = m.Match(".wh.Test1", mock2.NewFileInfo(), nil)
	assert.True(t, match)
	assert.False(t, extract)

	// Should match opaque directory.
	match, extract = m.Match(".wh..wh..opqTest1", mock2.NewFileInfo(), nil)
	assert.True(t, match)
	assert.False(t, extract)
}
...
func TestExecutableMatcher(t *testing.T) {
	...
	// Should match.
	match, extract = m.Match("Test1", mock2.NewFileInfo(mock2.FileMode(0001)), nil)
	assert.True(t, match)
	assert.False(t, extract)
	match, extract = m.Match("Test1", mock2.NewFileInfo(mock2.FileMode(0010)), nil)
	assert.True(t, match)
	assert.False(t, extract)
	match, extract = m.Match("Test1", mock2.NewFileInfo(mock2.FileMode(0011)), nil)
	assert.True(t, match)
	assert.False(t, extract)
	match, extract = m.Match("Test1", mock2.NewFileInfo(mock2.FileMode(0100)), nil)
	assert.True(t, match)
	assert.False(t, extract)
	match, extract = m.Match("Test1", mock2.NewFileInfo(mock2.FileMode(0101)), nil)
	assert.True(t, match)
	assert.False(t, extract)
	match, extract = m.Match("Test1", mock2.NewFileInfo(mock2.FileMode(0111)), nil)
	assert.True(t, match)
	assert.False(t, extract)
}
...
func TestRegexpMatcher(t *testing.T) {
	...
	assert.True(t, match)
	assert.False(t, extract)
	match, extract = m.Match("HelloTestGoodbye", mock2.NewFileInfo(), nil)
	assert.True(t, match)
	assert.False(t, extract)
...
func TestSymboliclinkMatcher(t *testing.T) {
	...
	// Should not match.
	match, extract := m.Match("Fail1", mock2.NewFileInfo(), nil)
	assert.False(t, match)
	assert.False(t, extract)
	match, extract = m.Match("Fail2", mock2.NewFileInfo(mock2.FileMode(0111)), nil)
	assert.False(t, match)
	assert.False(t, extract)

	// Should match.
	match, extract = m.Match("Test1", mock2.NewFileInfo(mock2.FileMode(os.ModeSymlink)), nil)
	assert.True(t, match)
	assert.False(t, extract)
}
...
func TestOrMatcher(t *testing.T) {
	...
	match, extract = m.Match("Test1", mock2.NewFileInfo(), nil)
	assert.True(t, match)
	assert.False(t, extract)
	match, extract = m.Match("Fail2", mock2.NewFileInfo(), nil)
	assert.False(t, match)
	assert.False(t, extract)

	// Should only be extractable if there is a match.
	m = NewOrMatcher(NewRegexpMatcher(regexp.MustCompile("Test"), true))
	match, extract = m.Match("Test2", mock2.NewFileInfo(), nil)
	assert.True(t, match)
	assert.True(t, extract)
	match, extract = m.Match("Fail3", mock2.NewFileInfo(), nil)
	assert.False(t, match)
	assert.False(t, extract)

	m = NewOrMatcher(NewRegexpMatcher(regexp.MustCompile("Test"), true), NewRegexpMatcher(regexp.MustCompile("test"), false))
	// Should match and be extractable.
	match, extract = m.Match("Test3", mock2.NewFileInfo(), nil)
	assert.True(t, match)
	assert.True(t, extract)
...
func TestAndMatcher(t *testing.T) {
	...
	match, extract = m.Match(".wh.Test3", mock2.NewFileInfo(), nil)
	assert.True(t, match)
	assert.False(t, extract)
	match, extract = m.Match("Fail4", mock2.NewFileInfo(), nil)
	assert.False(t, match)
	assert.False(t, extract)
}
