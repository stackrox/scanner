...
func (pgSQL *pgSQL) insertFeatureVersion(fv database.FeatureVersion) (id int, err error) {
	...
	t = time.Now()
	err = tx.QueryRow(insertFeatureVersion, featureID, fv.Version, pq.Array(fv.ProvidedExecutables), pq.Array(fv.ProvidedLibraries), StringMapArrayString(fv.NeededLibrariesMap)).Scan(&fv.ID)
