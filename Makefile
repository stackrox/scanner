...
scanner-image: scanner-build-dockerized ossls-notice $(CURDIR)/image/scanner/rhel/bundle.tar.gz
	@echo "+ $@"
	@docker build --target scanner -t scanner:$(TAG) -f image/scanner/rhel/Dockerfile image/scanner/rhel

.PHONY: scanner-image-slim
scanner-image-slim: scanner-build-dockerized ossls-notice $(CURDIR)/image/scanner/rhel/bundle.tar.gz
	@echo "+ $@"
	@docker build --target scanner-slim -t scanner-slim:$(TAG) -f image/scanner/rhel/Dockerfile image/scanner/rhel
...
db-image: $(CURDIR)/image/db/rhel/bundle.tar.gz
	...
	@docker build --target scanner-db -t scanner-db:$(TAG) -f image/db/rhel/Dockerfile image/db/rhel

.PHONY: db-image-slim
db-image-slim: $(CURDIR)/image/db/rhel/bundle.tar.gz
	@echo "+ $@"
	@test -f image/db/dump/definitions.sql.gz || { echo "FATAL: No definitions dump found in image/dump/definitions.sql.gz. Exiting..."; exit 1; }
	@docker build --target scanner-db-slim -t scanner-db-slim:$(TAG) -f image/db/rhel/Dockerfile image/db/rhel
...
deploy-dockerhub: clean-helm-rendered
	@echo "+ $@"
	kubectl create namespace stackrox || true
	helm template scanner chart/ --set tag=$(TAG),logLevel=$(LOGLEVEL),updateInterval=2m,scannerImage=quay.io/stackrox-io/scanner,scannerDBImage=quay.io/stackrox-io/scanner-db --output-dir rendered-chart
	kubectl apply -R -f rendered-chart
