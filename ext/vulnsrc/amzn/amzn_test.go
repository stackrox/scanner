...
func TestAmazonLinux2(t *testing.T) {
	...
	assert.Equal(t, "CVE-2017-5715", vulnerabilities[2].Name)
	assert.Equal(t, "https://nvd.nist.gov/vuln/detail/CVE-2017-5715", vulnerabilities[2].Link)
	assert.Equal(t, database.UnknownSeverity, vulnerabilities[2].Severity)
	assert.Equal(t, expectedDescription0, vulnerabilities[2].Description)
	assert.Equal(t, 18, len(vulnerabilities[2].FixedIn))

	assert.Equal(t, "CVE-2017-5754", vulnerabilities[3].Name)
	assert.Equal(t, "https://nvd.nist.gov/vuln/detail/CVE-2017-5754", vulnerabilities[3].Link)
	assert.Equal(t, database.UnknownSeverity, vulnerabilities[3].Severity)
	assert.Equal(t, expectedDescription0, vulnerabilities[3].Description)
	assert.Equal(t, 13, len(vulnerabilities[3].FixedIn))
