...
func (r *reloadProxyConfigHandler) OnStableUpdate(val interface{}, err error) {
	log.Infof("OnStableUpdate: Proxy config is %v, proxy read err is %v", val, err)
	if err != nil {
...
func (r *reloadProxyConfigHandler) updateEnvNoLock() {
	log.Infof("updateEnvNoLock: Possibly updating proxy environment. URL: %v, setEnv flag: %v", r.proxyURL, r.setEnv)
	if !r.setEnv {
		return
	}
	if r.proxyURL == nil {
...
func (r *reloadProxyConfigHandler) enableProxySetting(enable bool) {
	log.Infof("enableProxySetting: Changing setEnv flag to %v", r.setEnv)
	r.mutex.Lock()
	defer r.mutex.Unlock()
	r.setEnv = enable
	r.updateEnvNoLock()
